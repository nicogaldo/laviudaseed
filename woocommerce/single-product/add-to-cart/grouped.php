...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
    <div class="table-responsive">
        <table cellspacing="0" class="woocommerce-grouped-product-list group_table table table-striped-columns w-100">
            <tbody>
                <?php
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					echo '<td class="woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . ' align-middle">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
